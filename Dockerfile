...
ARG REGISTRY_VERSION=5.5.3-lkd-r0
...
ARG SECRET_PROVIDER_VERSION=2.1.5
...
## Kafka Connect JDBC
ARG KAFKA_CONNECT_JDBC_VERSION=5.5.3-lkd-r0
...
ARG KAFKA_CONNECT_ELASTICSEARCH_VERSION=5.5.3-lkd-r0
...
## Kafka Connect HDFS
ARG KAFKA_CONNECT_HDFS_VERSION=5.5.3-lkd-r0
...
# Kafka Connect S3
ARG KAFKA_CONNECT_S3_VERSION=5.5.3-lkd-r0
...
RUN wget "${ELASTICSEARCH_URL}" -O /elasticsearch.tar.gz \
    ...
    && sed -i '2iif [[ -z $JAVA_HOME ]]; then JAVA_HOME=/usr; fi # Added by andmarios' /opt/elasticsearch/bin/elasticsearch-env \
