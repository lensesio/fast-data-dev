...
ARG REGISTRY_VERSION=5.1.0-lkd-r0
...
RUN wget $DEVARCH_USER $DEVARCH_PASS "${STREAM_REACTOR_URL}" -O /stream-reactor.tar.gz \
    ...
    && for file in $(find /opt/landoop/connectors/stream-reactor -maxdepth 2 -type f -exec basename {} \; | grep -Ev "scala-logging|kafka-connect-common|scala-" | sort | uniq -c | grep -E "^\s+21 " | awk '{print $2}' ); do \
...
## Kafka Connect JDBC
ARG KAFKA_CONNECT_JDBC_VERSION=5.1.0-lkd-r0
...
ARG KAFKA_CONNECT_ELASTICSEARCH_VERSION=5.1.0-lkd-r0
...
## Kafka Connect HDFS
ARG KAFKA_CONNECT_HDFS_VERSION=5.1.0-lkd-r0
...
# Kafka Connect S3
ARG KAFKA_CONNECT_S3_VERSION=5.1.0-lkd-r0
