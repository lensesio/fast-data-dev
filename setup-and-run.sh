...
sed -e 's/2181/'"$ZK_PORT"'/' -e 's/8081/'"$REGISTRY_PORT"'/' -e 's/9092/'"$BROKER_PORT"'/' -i \
    /opt/confluent-3.0.1/etc/kafka/zookeeper.properties \
    /opt/confluent-3.0.1/etc/kafka/server.properties \
    /opt/confluent-3.0.1/etc/schema-registry/schema-registry.properties \
    /opt/confluent-3.0.1/etc/schema-registry/connect-avro-distributed.properties

## Broker specific
cat <<EOF >>/opt/confluent-3.0.1/etc/kafka/server.properties
...
# cat <<EOF >>/opt/confluent-3.0.1/etc/schema-registry/schema-registry.properties

# listeners=http://0.0.0.0:$REGISTRY_PORT
# EOF

## REST Proxy specific
cat <<EOF >>/opt/confluent-3.0.1/etc/kafka-rest/kafka-rest.properties
...
cat <<EOF >>/opt/confluent-3.0.1/etc/schema-registry/connect-avro-distributed.properties
