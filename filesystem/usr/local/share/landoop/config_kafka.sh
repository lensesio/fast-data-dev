...
OPTS_LITERAL="LENSES_KAFKA_CLIENT_SETTINGS_SASL_JAAS_CONFIG"
# Deprecated literals
...
OPTS_SKIP="LENSES_ZOOKEEPER_HOSTS LENSES_KAFKA_BROKERS LENSES_SCHEMA_REGISTRY_URLS LENSES_KAFKA_CONNECT_CLUSTERS LENSES_LICENSE_FILE"
    ...
    # Some settings should be skipped and not processed
    if [[ "$OPTS_SKIP" =~ " $var " ]]; then
        return 0
    fi
